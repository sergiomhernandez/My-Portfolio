  ...
  return (
    <div name="navbar">
      <nav className='navbar'>
        <div className="flex justify-center items-center w-full h-[60px] z-10 text-white fixed md:px-4 top-0 left-0 bg-black">
          <div className='flex justify-between md:max-w-6xl w-[90%] '>
              <a className='text-xl tracking-[0.20em] ml-2 uppercase font-bold list-none cursor-pointer'>Sergio</a>
            <ul className="hidden md:flex">
              {links.map(({id, link}) => (
                <li key={id} 
                className="px-4 list-item cursor-pointer capitalize font-medium text-white hover:scale-105 hover:text-teal-400 duration-200"
                >
